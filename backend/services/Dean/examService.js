...
const { Exam, Instructor, Course, Proctoring, User, TeachingAssistant, Classroom } = require('../../models');
...
class ExamService {
    ...
    async createExam(examData) {
        ...
        try {
            ...
            const exam = await Exam.create({
                ...
                departments: examData.departments,
            }, { transaction: t });
    ...
    async getExamsWithProctors() {
        try {
          ...
          // 2) iterate, fetch proctors, build plain objects
          const result = [];
          for (const exam of exams) {
            const plain = exam.toJSON();
    
            // pull in your helper
            const proctors = await this.getProctorsForExam(exam.id);
    
            result.push({
              ...plain,
              proctors,
              swapCount: plain.swapCount || 0
            });
          }
    ...
    async assignProctors(options) {
        ...
        try {
            ...
            availableTAs = availableTAs.filter(ta =>
                departments.includes(ta.department) || ta.isMultidepartmentExam === true
            );
