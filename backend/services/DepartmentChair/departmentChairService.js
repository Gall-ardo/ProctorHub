...
class DepartmentChairService {
     ...
     */
    async getChairProfile(chairId) {
        try {
            console.log('Fetching profile for chair ID:', chairId);
            
            // Find the chair without joins first
            const chair = await DepartmentChair.findByPk(chairId);

            if (!chair) {
                console.log(`No chair found with ID: ${chairId}`);
            ...
            console.log(`Found chair with department: ${chair.department}`);
            
            // Try to get the user info if available
            try {
                const user = await User.findByPk(chairId);
                return {
                    id: chairId,
     ...
     */
    async getChairDepartment(chairId) {
        try {
            console.log('Fetching department for chair ID:', chairId);
            
            const chair = await DepartmentChair.findByPk(chairId);

            if (!chair) {
                console.log(`No chair found with ID: ${chairId}, returning default department`);
                return 'Computer Science'; // Default department for development
            }

            console.log(`Found department: ${chair.department}`);
            return chair.department;
        } catch (error) {
     ...
     * @param {string|number} courseId - The ID of the course
     * @param {Array<string|number>} taIds - Array of TA IDs to assign
...
module.exports = new DepartmentChairService(); 