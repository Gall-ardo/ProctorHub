...
import './InstructorTAAssignerPage.css';
import InstructorNavBar from './InstructorNavBar';
import axios from 'axios';

function InstructorTAAssignerPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const instructorData = await axios.get(`${API_URL}/instructor/profile`, { headers });
                const department = instructorData.data.data?.department;
                ...
                const [coursesResponse, tasResponse, requestsResponse] = await Promise.all([
                    axios.get(`${API_URL}/instructor/department-courses/${department}`, { headers }),
                    axios.get(`${API_URL}/instructor/available-tas`, { headers }),
    ...
    const handleCourseSelect = (course) => {
        ...
        if (course) {
            try {
                ...
                const headers = getAuthHeader();
                console.log(`DEBUGGING: Making API call to ${API_URL}/instructor/course-tas/${course.id}`);
                axios.get(`${API_URL}/instructor/course-tas/${course.id}`, { headers })
                    .then(response => {
    ...
    const handleFinalize = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                `${API_URL}/instructor/assign-tas-to-course`,
                assignmentData,
                { headers }
            );
        ...
        <div className="instructor-assign-page">
            ...
            {selectedCourse && (
                        <div className="selected-tas-panel">
                            <h2>Selected TAs for {selectedCourse.courseCode}</h2>
...
export default InstructorTAAssignerPage;